...
1. Call `qdrant-find` with `collection_name={{PLAN_NAME}}`, as needed, to retrieve the triage root cause, prior fix attempts, verification failure output, and any code conventions relevant to the fix.
